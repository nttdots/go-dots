 ...
 */
 func (v *goAristaAclValidator) ValidateMandatoryAttributes(name string, matches *types.Matches) (bool, string) {
	if matches.IPv4 != nil && (matches.IPv4.Protocol == nil || matches.IPv4.DestinationIPv4Network == nil ||
		matches.IPv4.SourceIPv4Network == nil || matches.IPv4.Fragment == nil) {
		errorMsg := fmt.Sprintf("Body Data Error : Missing required acl's attributes as 'protocol', 'destination-ipv4-network', 'source-ipv4-network', 'fragment' at acl 'name' = %+v", name)
		log.Error(errorMsg)
		return false, errorMsg
	} else if  matches.IPv6 != nil && (matches.IPv6.Protocol == nil || matches.IPv6.DestinationIPv6Network == nil || matches.IPv6.SourceIPv6Network == nil) {
		errorMsg := fmt.Sprintf("Body Data Error : Missing required acl's attributes as 'protocol', 'destination-ipv6-network', 'source-ipv6-network' at acl 'name' = %+v", name)
		log.Error(errorMsg)
		return false, errorMsg
	}
	...
	if matches.TCP != nil && (matches.TCP.FlagsBitmask == nil || matches.TCP.SourcePort == nil || matches.TCP.DestinationPort == nil) {
		...
		log.Error(errorMsg)
		return false, errorMsg
	} else if matches.ICMP != nil && matches.ICMP.Type == nil {
		errorMsg := fmt.Sprintf("Body Data Error : Missing required acl's attributes as 'type' at acl 'name' = %+v", name)
		log.Error(errorMsg)
		return false, errorMsg
	}
