...
func ValidateTelemetryConfiguration(customerID int, cuid string, tsid int, data *messages.TelemetryConfigurationCurrent) (isPresent bool, isUnprocessableEntity bool, errMsg string) {
	...
	for _, config := range data.UnitConfigList {
		...
		if *config.Unit != int(PacketsPerSecond) && *config.Unit != int(KiloPacketsPerSecond) && *config.Unit != int(BitsPerSecond) &&
		   *config.Unit != int(KilobytesPerSecond) && *config.Unit != int(MegabytesPerSecond) && *config.Unit != int(GigabytesPerSecond) {
			errMsg = fmt.Sprintf("Invalid unit value: %+v. Expected values include 1:pps, 2:kilo-ps, 3:bps, 4:kilobytes-ps, 5:megabytes-ps, 6:gigabytes-ps", *config.Unit)
			log.Error(errMsg)
			isUnprocessableEntity = true
			return
		}
...
func ValidateBaseline(customer *Customer, cuid string, tsid int, data []messages.Baseline) (isPresent bool, isUnprocessableEntity bool, errMsg string) {
	...
	for _, v := range data {
		...
		// Validate protocol
		errMsg = ValidateProtocol(v.TargetProtocol)
		if errMsg != "" {
			log.Error(errMsg)
			isUnprocessableEntity = true
			return
...
// Validate protocol
func ValidateProtocol(protocolList []int) (errMsg string) {
